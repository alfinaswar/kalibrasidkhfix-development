...
@section('content')
    ...
    <script>
        ...
        $('.flowinput').on('input', function() {
            var pengulangan = [];
            for (var i = 1; i <= 6; i++) {
                pengulangan[i] = parseFloat($(this).closest('tr').find(
                    'input[name="Pengulangan' + i + '[]"]').val());
            }
            ...
            pengulangan.forEach((value, index) => {
                ...
                if (value < batasBawah || value > batasAtas) {
                    $(this).closest('tr').find(
                        'input[name="Pengulangan' + (index) + '[]"]').addClass(
                        'is-invalid');

                } else {
                    $(this).closest('tr').find(
                        'input[name="Pengulangan' + (index) + '[]"]').removeClass(
                        'is-invalid');
                }
            });
        });
        $('.occinput').on('input', function() {
            var pengulangan = [];
            for (var i = 1; i <= 6; i++) {
                pengulangan[i] = parseFloat($(this).closest('tr').find(
                    'input[name="PengulanganOcc' + i + '[]"]').val());
            }
            var titikUkur = parseFloat($(this).closest('tr').find('input[name="SettingAlatOcc[]"]').val());

            var batasBawah = titikUkur - 20;
            var batasAtas = titikUkur + 20;

            pengulangan.forEach((value, index) => {

                if (value < batasBawah || value > batasAtas) {
                    $(this).closest('tr').find(
                        'input[name="PengulanganOcc' + (index) + '[]"]').addClass(
                        'is-invalid');

                } else {
                    $(this).closest('tr').find(
                        'input[name="PengulanganOcc' + (index) + '[]"]').removeClass(
                        'is-invalid');
                }
            });
        });
