...
class TimbanganBayiMekanikController extends Controller
{
    ...
    public function store($data)
    {
        ...
        // Pengukuran Kinerja
        $pengukurankinerja = SertifikatTimbanganPengujian::updateOrCreate(
            [
                'SertifikatId' => $data['sertifikatid'],
            ...
            [
                'InstrumenId' => $data['idinstrumen'] ?? null,
                'MassaHalf' => array_slice($data['MassaHalf'], 0, 10) ?? null,
                'PengujianZ' => array_slice($data['PengujianZ'], 0, 10) ?? null,
                'PengujianM' => array_slice($data['PengujianM'], 0, 10) ?? null,
                'idUser' => auth()->user()->id
            ]
        );

        $pengukuranSkala = SertifikatTimbanganPengujian::updateOrCreate(
            [
                'SertifikatId' => $data['sertifikatid'],
                'TipePengujian' => 'SKALA'
            ],
