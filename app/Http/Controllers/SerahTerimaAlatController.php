...
<div class="btn-group">
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($request->InstrumenId as $key => $value) {
            ...
            $cekDatakaji = KajiUlang::where('SerahTerimaId', $id)->get();
            // dd(empty($cekDatakaji));
            if ($cekDatakaji->isEmpty()) {
                // dd('kosong sini');
    ...
    {
        $st = SerahTerima::with('getQuotation')->find($id);
        if ($st) {
            if ($st->getQuotation()->count() > 0) {
                return response()->json(['message' => 'Tidak dapat dihapus karena memiliki data quotation'], 404);
            } else {
                $st->delete();
                $st->Stdetail()->delete();
                $st->dataKaji()->delete();
